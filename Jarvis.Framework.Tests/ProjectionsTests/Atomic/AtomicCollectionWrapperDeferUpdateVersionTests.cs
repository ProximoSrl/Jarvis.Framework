...
namespace Jarvis.Framework.Tests.ProjectionsTests.Atomic
{
    ...
    public class AtomicCollectionWrapperDeferUpdateVersionTests : AtomicCollectionWrapperTestsBase
    {
        ...
        #region StopAndFlushAllAsync (Coordinator) Tests

        [Test]
        public async Task StopAndFlushAll_should_drain_pending_items_across_wrappers()
        {
            ...
            // Act: Call the static coordinator shutdown
            await DeferredUpdateVersionCoordinator.StopAndFlushAllAsync().ConfigureAwait(false);
            ...
            Assert.That(reloaded, Is.Not.Null);
            Assert.That(reloaded.AggregateVersion, Is.EqualTo(rm.AggregateVersion),
                "StopAndFlushAllAsync should drain all pending deferred updates");
        }

        [Test]
        public async Task StopAndFlushAll_with_no_registered_pipelines_should_not_throw()
        {
            // Act & Assert: Should be a safe no-op
            Assert.DoesNotThrowAsync(() => DeferredUpdateVersionCoordinator.StopAndFlushAllAsync());
        }
