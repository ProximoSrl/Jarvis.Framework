namespace Jarvis.Framework.Tests.EngineTests.Unfolder
{
    ...
    //{
    //    private TestAggregateQueryModel sut;
    ...
    //    private AggregateFactoryEx _aggregateFactory = new AggregateFactoryEx(null);
    ...
    //            .Returns(NStoreNullLogger.Instance);
    ...
    //        sut = new QueryModelRepository(_factory, _persistence, _snapshotStore);
    //    }
    ...
    //        using (var stream = _persistence.OpenStream("BLAH", id.AsString(), 0, Int32.MaxValue))
    //        {
    ...
    //        }
    //        _snapshotStore.Load(null, 1, null).ReturnsForAnyArgs(s);
    ...
    //    {
    //        var id = new TestEventUnfolderId(Interlocked.Increment(ref progressive));
    //        using (var stream = _persistence.OpenStream(id.AsString()))
    //        {
    //            CommitOneValue(stream, 42);
    //        }
    ...
    //        _snapshotStore.Received().Persist(Arg.Any<ISnapshot>(), Arg.Any<String>());
    //    }
