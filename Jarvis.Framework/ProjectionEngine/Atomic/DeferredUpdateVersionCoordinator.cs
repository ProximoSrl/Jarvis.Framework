...
namespace Jarvis.Framework.Kernel.ProjectionEngine.Atomic
{
    ...
    /// </para>
    /// </summary>
    internal static class DeferredUpdateVersionCoordinator
    {
        ...
        /// <summary>
        /// Stops the shared timer, triggers all pending batches, completes all
        /// pipelines, and clears registrations. This must be called during application
        /// shutdown to guarantee no data is lost.
        /// </summary>
        internal static async Task StopAndFlushAllAsync()
        {
            // 1. Stop the timer so no more periodic triggers fire
            StopTimer();

            // 2. Snapshot current registrations and trigger + complete all
            var pipelines = _registeredPipelines.Values.ToList();
            ...
            foreach (var pipeline in pipelines)
            {
                ...
                {
                    _logger.ErrorFormat(ex, "Error triggering batch during shutdown flush");
                }
            }

            // 4. Complete and wait for all pipelines to drain
            var completionTasks = new List<Task>();
            foreach (var pipeline in pipelines)
            {
                try
                {
                    completionTasks.Add(pipeline.CompleteAndWait());
                }
                catch (Exception ex)
                {
                    _logger.ErrorFormat(ex, "Error completing pipeline during shutdown flush");
                }
            }
