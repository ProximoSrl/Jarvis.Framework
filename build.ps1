...
if ($CreateNugetPackages) {
    ...
    dotnet pack "Jarvis.Framework.Kernel/Jarvis.Framework.Kernel.csproj" --no-build --no-dependencies -p:PackageId=Jarvis.Framework -o $publishDirectory --configuration $Configuration /p:PackageVersion=$nugetVersion /p:assemblyVersion=$assemblyVersion /p:FileVersion=$assemblyFileVersion /p:InformationalVersion=$assemblyInformationalVersion -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg
    Assert-LastExecution -message "Unable to create nuget for Jarvis.Framework.Kernel" -haltExecution $true

    dotnet pack "Jarvis.Framework.Bus.Rebus.Integration/Jarvis.Framework.Bus.Rebus.Integration.csproj" --no-build --no-dependencies -p:PackageId=Jarvis.Framework.Rebus -o $publishDirectory --configuration $Configuration /p:PackageVersion=$nugetVersion /p:assemblyVersion=$assemblyVersion /p:FileVersion=$assemblyFileVersion /p:InformationalVersion=$assemblyInformationalVersion -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg
