...
{
    public class MongoImporterSerilog : BaseImporter
    {
        ...
        protected override PollResult OnPoll()
        {
            ...
            resultList = _collection
                .Find(Builders<BsonDocument>.Filter.And(
                        Builders<BsonDocument>.Filter.Gte(TimestampFieldName, _checkpoint.LastCheckpoint),
                        Builders<BsonDocument>.Filter.Lt(TimestampFieldName, limitDate)))
            ...
            DateTime lastCheckpoint = resultList.Select(b => b[TimestampFieldName].ToUniversalTime()).Max();
            var ready = resultList.Select(b =>
            {
                try
                {
                    ...
                    DateTime timestamp = b[TimestampFieldName].ToUniversalTime();
                    b[TimestampFieldName] = timestamp.ToString(DateTimeFormatForNestQuery, CultureInfo.InvariantCulture);
                    b["collection"] = Collection;
                    b["mongo-server"] = Connection;
                    b["source"] = Connection + "/" + Collection;
                    var level = b["Level"].AsString;

                    //TODO: Fix the ttl... something is not right.
                    String ttl = "30d";
                    if (level == "DEBUG")
                    {
                        ttl = "2d";
                ...
                {
                    _log.Error("Error converting mongo log: " + b, ex);
                    return null;
                }
            });
