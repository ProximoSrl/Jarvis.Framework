...
namespace Jarvis.Framework.Shared.ReadModel.Atomic
{
    ...
    public abstract class AbstractAtomicReadModel : IAtomicReadModel
    {
        ...
        public List<long> LastProcessedVersions
        {
            ...
            private set => _lastProcessedVersions = value;
        }
        ...
        private List<long> _lastProcessedVersions;
        ...
        public virtual Boolean ProcessChangeset(Changeset changeset)
        {
            ...
            //We need to be conservative for existing aggregates that never processed one last version
            if (changeset.AggregateVersion < this.AggregateVersion)
            {
                var versionWasNotProcessed = !LastProcessedVersions.Contains(changeset.AggregateVersion) && LastProcessedVersions.Count > 0;
                ...
                if (versionWasNotProcessed && changeset.AggregateVersion > this.AggregateVersion - MaxNumberOfVersionToKeep)
                {
